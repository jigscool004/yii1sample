...
class JSONStorage extends CComponent {
    ...
    public function verify() {
        $registry = function_exists('json_encode') ? json_encode($this->data[self::REGISTRY]) : CJSON::encode(
    ...
    private function flush() {
        ...
        if (file_put_contents($this->getFile(), $this->encode($data))) {
            ...
            throw new Exception(strtr(
                    'Unable to write back to {FILE}. Data will be lost!', array('{FILE}' => $this->getFile())
            ));
        }
    }
