    ...
    public function init() {
        if ($this->name === null) {
            throw new CException(Yii::t(
                    'zii', '"{attribute}" attribute cannot be empty.', array('{attribute}' => "name")
            ));
        }
...
function() {
    ...
    protected function renderDataCellContent($row, $data) {
        ...
        $checked = ($this->value === null) ? CHtml::value($data, $this->name) : $this->evaluateExpression($this->value, array('data' => $data, 'row' => $row));
        ...
        $button['url'] = isset($button['url']) ? $this->evaluateExpression(
                        $button['url'], array('data' => $data, 'row' => $row)
                ) : '#';
