...
class TbBulkActions extends CComponent {
            ...
            if (preg_match('/ccheckboxcolumn/i', $column['class'])) {
                if (isset($column['checkBoxHtmlOptions']) && isset($column['checkBoxHtmlOptions']['name'])) {
                    $this->columnName = strtr(
                            $column['checkBoxHtmlOptions']['name'], array('[' => "\\[", ']' => "\\]")
                    );
                } else {
    ...
    public function renderButtons() {
        ...
        echo CHtml::openTag(
                'div', array('id' => $this->getId(), 'style' => 'position:relative', 'class' => $this->align)
        );
    ...
    private function convertToTbButtonConfig($action) {
        ...
        if (!isset($action['id'])) {
            throw new CException(Yii::t(
                    'zii', 'Each bulk action button should have its "id" attribute set to ensure its functionality among ajax updates'
            ));
        }
