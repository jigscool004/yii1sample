...
class TbExtendedFilter extends CWidget {
    ...
    public function init() {
        ...
        $this->filteredBy = array_filter(
                $this->model->getAttributes(), function ($i) {
            return $i != null;
        }
        );
    ...
    public function registerClientScript() {
        ...
        Yii::app()->clientScript->registerScript(
                __CLASS__ . '#extended-filter' . $this->grid->id, <<<EOD
...
EOD
    ...
    protected function displaySavedFilters($registryKey) {
        ...
        if ($this->jsonStorage->getLength($this->registry)) {
            ...
            echo CHtml::link(
                    '<i class="glyphicon glyphicon-ok glyphicon glyphicon-white"></i>', '#', array('class' => 'btn btn-primary btn-extended-filter-apply')
            );
            echo '&nbsp;';
