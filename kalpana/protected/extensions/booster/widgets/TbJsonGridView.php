...
class TbJsonGridView extends TbGridView {
    ...
    public function renderItems() {
        if ($this->json) {
            echo function_exists('json_encode') ? json_encode($this->renderTableBody()) : CJSON::encode(
    ...
    protected function renderTemplates() {
        ...
        echo $this->renderTemplate(
                $this->id . '-row-template', '<tr class="<%=this.class%>"><% var t = "#' . $this->id . '-col-template"; out += $.jqote(t, this.cols);%></tr>'
        );
        echo $this->renderTemplate($this->id . '-keys-template', '<span><%=this%></span>');

        echo $this->renderTemplate(
                $this->id . '-pager-template', '<li class="<%=this.class%>"><a href="<%=this.url%>"><%=this.text%></a></li>'
        );

        echo $this->renderTemplate(
                $this->id . '-summary-template', '<div class="<%=this.class%>"><%=this.text%></div>'
        );
    }
    ...
    public function renderTableRow($row) {
        if ($this->rowCssClassExpression !== null) {
            ...
            echo '<tr class="' . $this->evaluateExpression(
                    $this->rowCssClassExpression, array('row' => $row, 'data' => $data)
            ) . '">';
