...
class TbSortableAction extends CAction {
    ...
    public function run() {
        ...
        if (!$model->hasAttribute($sortableAttribute)) {
            throw new CHttpException(400, Yii::t(
                    'yii', '{attribute} "{value}" is invalid.', array('{attribute}' => 'sortableAttribute', '{value}' => $sortableAttribute)
            ));
        }
    ...
    private function isValidRequest() {
        return Yii::app()->request->isPostRequest && Yii::app()->request->isAjaxRequest && isset($_POST['sortOrder']);
    }
